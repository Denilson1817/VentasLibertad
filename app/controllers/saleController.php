...
class saleController extends mainModel
{
	...
	public function agregarProductoCarritoControlador()
	{
		...
		if (empty($_SESSION['datos_producto_venta'][$codigo])) {
			...
			if ($stock_total < 0) {
				...
				// 	"icono"=>"error"
				// ];
				$stock_total = 0; // Permitir agregar productos sin inventario
				// return json_encode($alerta);
				exit();
			}
	...
	/*---------- Controlador agregar producto sin inventario a venta ----------*/
	public function agregarProductoSinInventarioCarritoControlador()
	{
		/*== Validar que se reciban los datos correctos ==*/
		if (empty($_POST['producto_nombre']) || empty($_POST['producto_precio_venta'])) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "Debe ingresar el nombre y el precio de venta del producto",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		...
		$precio_compra = 0; // Valor predeterminado para precio de compra
		$precio_venta = floatval($_POST['producto_precio_venta']);
		$stock_total = isset($_POST['producto_stock_total']) ? intval($_POST['producto_stock_total']) : 0;
		$tipo_unidad = "unidad";
		$marca = "";
		$modelo = "";
		$estado = "activo"; // Proporciona un valor para producto_estado
		$categoria_id = 1; // Valor predeterminado para categoria_id

		/*== Generar un código de barras único ==*/
		do {
			$codigo = uniqid("PROD_"); // Agregamos un prefijo para diferenciar
			$check_producto = $this->ejecutarConsulta("SELECT producto_codigo FROM producto WHERE producto_codigo='$codigo'");
		} while ($check_producto->rowCount() > 0); // Asegurar que el código sea único

		/*== Insertar el nuevo producto en la base de datos ==*/
		$query = "INSERT INTO producto (producto_codigo, producto_nombre, producto_precio_compra, producto_precio_venta, producto_stock_total, producto_tipo_unidad, producto_marca, producto_modelo, producto_estado, categoria_id) VALUES ('$codigo', '$nombre', $precio_compra, $precio_venta, $stock_total, '$tipo_unidad', '$marca', '$modelo', '$estado', $categoria_id)";
		$insert_producto = $this->ejecutarConsulta($query);

		if (!$insert_producto) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "No se pudo registrar el producto en la base de datos.",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		...
		/*== Recuperar datos del producto recién insertado ==*/
		$check_producto = $this->ejecutarConsulta("SELECT * FROM producto WHERE producto_codigo='$codigo'");
		$campos = $check_producto->fetch();
		...
		$_SESSION['datos_producto_venta'][$codigo] = [
			...
			"producto_stock_total" => $campos['producto_stock_total'],
			"producto_stock_total_old" => $campos['producto_stock_total'],
			"venta_detalle_precio_compra" => $campos['producto_precio_compra'],
			"venta_detalle_precio_venta" => $campos['producto_precio_venta'],
			"venta_detalle_cantidad" => 1,
			"venta_detalle_total" => number_format($campos['producto_precio_venta'], MONEDA_DECIMALES, '.', ''),
			"venta_detalle_descripcion" => $campos['producto_nombre']
		];
