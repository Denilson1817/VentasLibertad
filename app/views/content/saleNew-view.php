            ...
            <div class="column pb-6">
                ...
                <form class="pt-6 pb-6" id="sale-barcode-form" autocomplete="off">
                    <div class="columns">
                        ...
                        <div class="column is-one-quarter">
                            <button type="button" class="button is-link is-light js-modal-trigger" data-target="modal-js-product-sin-inventario"><i class="fas fa-plus"></i> &nbsp; Productos sin inventaro</button>
                        </div>
                        <div class="column">
...
<!-- Modal agregar producto a la lista de ventas sin inventario-->
<div class="modal" id="modal-js-product-sin-inventario">
    <div class="modal-background"></div>
    <div class="modal-card">
        <header class="modal-card-head">
            <p class="modal-card-title is-uppercase">
                <i class="fas fa-plus"></i> &nbsp; Agregar producto sin inventario
            </p>
            <button class="delete" aria-label="close"></button>
        </header>
        <section class="modal-card-body">
            <form id="form-producto-sin-inventario">
                <!-- Nombre, marca, modelo -->
                <div class="field">
        ...
        <footer class="modal-card-foot">
            <button type="button" class="button is-success" onclick="agregarProductoSinInventario()">
                Agregar a la venta
            </button>
    ...
    function agregar_producto() {
        ...
        if (codigo_producto != "") {
            ...
            Swal.fire({
                icon: 'warning',
                title: 'Alerta',
                text: 'Debes de introducir el código del producto para poder agregarlo',
                confirmButtonText: 'Aceptar'
            });
        }
    }

    /* Agregar producto sin inventario */
    function agregarProductoSinInventario() {
        // Obtiene y limpia los datos del formulario del modal
        let nombre = document.querySelector('#producto_nombre').value.trim();
        let precio = document.querySelector('#producto_precio').value.trim();
        let cantidad = document.querySelector('#producto_cantidad').value.trim();

        // Validación: Verifica que los campos no estén vacíos
        if (nombre !== "" && precio !== "" && cantidad !== "") {
            let datos = new FormData();
            datos.append("producto_nombre", nombre);
            datos.append("producto_precio_venta", precio);
            datos.append("producto_cantidad", cantidad);
            datos.append("modulo_venta", "agregarProductoSinInventario");
            ...
            fetch('<?php echo APP_URL; ?>app/ajax/ventaAjax.php', {
                ...
                .then(respuesta => respuesta.json())
                .then(respuesta => {
                    // Muestra alertas o mensajes según la respuesta
                    alertas_ajax(respuesta);

                    // Si el servidor retorna la tabla actualizada, la inserta en el contenedor
                    if (respuesta.tabla) {
                        document.querySelector('#tabla_productos').innerHTML = respuesta.tabla;
                    }

                    // Reinicia el formulario del modal
                    document.querySelector('#form-producto-sin-inventario').reset();

                    // Cierra el modal (simulando el click en el botón de cierre)
                    document.querySelector('#modal-js-product-sin-inventario .delete').click();
                })
            ...
            Swal.fire({
                icon: 'warning',
                title: 'Alerta',
                text: 'Debes completar todos los campos del producto sin inventario',
                confirmButtonText: 'Aceptar'
            });
        }
    }

    function agregarProductoAVentas() {
        // Obtener los valores del modal
        let nombreProducto = document.querySelector('#nombre_producto').value;
        let precioProducto = document.querySelector('#precio_producto').value;
        let cantidadProducto = document.querySelector('#cantidad_producto').value;

        // Validar que los campos no estén vacíos
        if (nombreProducto && precioProducto && cantidadProducto) {
            // Obtener la tabla de ventas
            let tablaVentas = document.querySelector('#tabla_ventas');

            // Crear una nueva fila
            let nuevaFila = document.createElement('tr');

            // Crear celdas para nombre, precio, cantidad y total
            ...
            let total = parseFloat(precioProducto) * parseInt(cantidadProducto);
            ...
            document.querySelector('#precio_producto').value = '';
